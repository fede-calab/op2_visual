...
int ball_tracking(ros::Publisher& ball_pub, bool no_display)
{
    ...
    std::string yamlFilePath = "/home/federico/Desktop/catkin_ws_prova/src/op2_visual/calibration/logitech_c270_hd_webcam/logitech_c270_hd_webcam_params.yml";
    ...
    // Open shared memory
    double* shared_value = nullptr;
    sem_t* mutex         = nullptr;
    if (!openSharedMemory(shared_value, mutex)) {
    ...
    std::string csvFilePath = "/home/federico/Desktop/catkin_ws_prova_due/src/op2_visual/data/ball_tracking_data.csv";
    ...
    while (g_run.load()) {
        ...
        if (haveMarker0 && haveMarker1) {
            ...
            // Read offset ratio from shared memory
            offsetRatio = (float)readSharedMemoryValue(shared_value, mutex);
    ...
    closeSharedMemory(shared_value, mutex);
